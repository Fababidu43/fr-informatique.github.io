...
const lastSignificantUpdate = "2026-09-01";
...
const services = [
  {
    ...
    nav: "Sites vitrines",
    icon: "bi-window",
    title: "Création de site internet vitrine en Haute-Loire",
  ...
  },
  {
    ...
    icon: "bi-tools",
    title: "Logiciels sur mesure en Haute-Loire",
  ...
  },
  {
    ...
    icon: "bi-database",
    title: "Bases de données et SQL en Haute-Loire",
...
function footer() {
  return `<footer class="site-footer">
    <div class="wrap">
      ...
      <div class="footer-bottom"><span>© ${new Date().getFullYear()} FR Informatique. Tous droits réservés.</span><span>Sites vitrines · Automatisations · Outils sur mesure · Dépannage local</span></div>
    </div>
  </footer><a class="scroll-top" href="#" aria-label="Retour en haut"><i class="bi bi-arrow-up" aria-hidden="true"></i></a>`;
}

function head({ title, description, url, schema, image = "hero-bg.webp", imageWidth = 1920, imageHeight = 1280, imageAlt = "FR Informatique, développeur indépendant à Beauzac" }) {
  ...
  <link rel="icon" href="assets/img/favicon.png" sizes="32x32" type="image/png"><link rel="apple-touch-icon" href="assets/img/apple-touch-icon.png" sizes="180x180">
...
const localBusiness = {
  ...
  logo: { "@type": "ImageObject", url: `${domain}/assets/img/apple-touch-icon.png`, width: 180, height: 180 },
...
function realisationsPage() {
  ...
  return `${head({ title: "Réalisations de sites vitrines en Haute-Loire", description, url: slug, schema: standardSchema("Réalisations FR Informatique", description, slug, "CollectionPage"), image: "meca-europe-new.webp", imageWidth: 1440, imageHeight: 980, imageAlt: "Comparaison avant et après de la refonte du site Meca Europe" })}<body data-page-act="2">${header(slug)}<main id="contenu">
...
function benefitBlock(service, variant, soft) {
  ...
  return `<section class="section${soft ? " section-soft" : ""}"><div class="wrap"><div class="section-head"><div><span class="eyebrow">Bénéfices</span><h2>Ce que cela change au quotidien.</h2></div></div><div class="${gridClass}">${service.benefits.map(([icon,title,text]) => `<article class="benefit-card" data-reveal><i class="bi ${icon}" aria-hidden="true"></i><h3>${title}</h3><p>${text}</p></article>`).join("")}</div></div></section>`;
}
...
const sitemapPages = ["index.html", "realisations.html", "a-propos.html", "contact.html", ...services.map((service) => service.slug)];
...
console.log(`Generated index.html, ${services.length} service pages, 3 dedicated navigation pages, 404.html and sitemap.xml.`);
